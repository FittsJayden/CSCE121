...
Image::Image(const Image& src) : image(nullptr), width(src.width), height(src.height) {
    allocateImage(width, height);

    for (unsigned int row = 0; row < src.height; row++) {
        for (unsigned int col = 0; col < src.width; col++) {
            image[col][row].setRed(src.image[col][row].getRed());
            image[col][row].setGreen(src.image[col][row].getGreen());
            image[col][row].setBlue(src.image[col][row].getBlue());
        }
    }
}
...
const Image& Image::operator=(const Image& src) {
    ...
    this->allocateImage(width, height);
