...
void processPostsByUser(Network& cppeers) {
    ...
    for (unsigned int i = 0; i < userPosts.size(); i++) {
        cout << userPosts[i]->getPostText().erase(0,1) << endl;
    }
...
void processPostsWithHashtags(Network& cppeers) {
    ...
    vector<Post*> taggedPosts = cppeers.getPostsWithTag(hashtag);
    for (unsigned int i = 0; i < taggedPosts.size(); i++) {
        cout << taggedPosts[i]->getPostText().erase(0,1) << endl;
    }
}
...
void processMostPopularHashtag(Network& cppeers) {
    ...
    for (unsigned int i = 0; i < popTags.size(); i++) {
        cout << popTags[i] << endl;
    }
}
