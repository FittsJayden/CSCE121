...
int main() {
  ...
  unsigned int th = targetHeight;
  unsigned int tw = targetWidth;

  if (th > height || tw > width || th < 1 || tw < 1) {
...
while ((width - targetWidth > 0) || (height - targetHeight > 0)) {   // find and remove a vertical seam
